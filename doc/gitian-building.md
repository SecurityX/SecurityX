...
*Setup instructions for a gitian build of SecurityX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SecurityX
...
- [Building SecurityX](#building-securityx)
...
Clone the git repositories for securityx and gitian and then checkout the securityx version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/securityx-crypto/securityx.git
cd securityx
...
Follow the instructions in [doc/release-process.md](release-process.md) in the securityx repository
...
To build SecurityX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the securityx repository.
...
```bash
    ...
    From https://github.com/securityx-crypto/securityx
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/securityx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/securityx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/securityx-build.assert
```
...
[securityx/gitian.sigs](https://github.com/securityx-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
