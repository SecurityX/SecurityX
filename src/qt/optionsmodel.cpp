...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SECX);
    ...
    if (!settings.contains("nAnonymizeSecurityXAmount"))
        settings.setValue("nAnonymizeSecurityXAmount", 1000);

    nAnonymizeSecurityXAmount = settings.value("nAnonymizeSecurityXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSecurityXAmount"))
        SoftSetArg("-anonymizesecurityxamount", settings.value("nAnonymizeSecurityXAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in securityx.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSecurityXAmount:
            return QVariant(nAnonymizeSecurityXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeSecurityXAmount:
            nAnonymizeSecurityXAmount = value.toInt();
            settings.setValue("nAnonymizeSecurityXAmount", nAnonymizeSecurityXAmount);
            emit anonymizeSecurityXAmountChanged(nAnonymizeSecurityXAmount);
            break;
        case CoinControlFeatures:
