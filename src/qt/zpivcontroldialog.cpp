...
#include "ui_zsecxcontroldialog.h"
...
std::set<std::string> ZSecxControlDialog::setSelectedMints;
std::set<CMintMeta> ZSecxControlDialog::setMints;

bool CZSecxControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSecxControlDialog::COLUMN_DENOMINATION || column == ZSecxControlDialog::COLUMN_VERSION || column == ZSecxControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSecxControlDialog::ZSecxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSecxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSecxControlDialog::~ZSecxControlDialog()
{
    delete ui;
}

void ZSecxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSecxControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSecxControlWidgetItem* itemDenom(new CZSecxControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSecxControlWidgetItem *itemMint = new CZSecxControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZSecxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSecxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSecx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSecxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSecxControlDialog::GetSelectedMints()
{
...
void ZSecxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
