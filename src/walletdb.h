...
public:
    ...
    bool WriteZSECXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSECXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSECXSeed_deprecated(uint256& seed);
    bool EraseZSECXSeed();
    bool EraseZSECXSeed_deprecated();

    bool WriteZSECXCount(const uint32_t& nCount);
    bool ReadZSECXCount(uint32_t& nCount);
