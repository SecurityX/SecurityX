...
#ifndef SecurityX_ZSECXWALLET_H
#define SecurityX_ZSECXWALLET_H
...
public:
    CzSECXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSECX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSECX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SecurityX_ZSECXWALLET_H
