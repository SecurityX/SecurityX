...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSecxStake(spend));
    } else {
        ...
        CSecxStake* secxInput = new CSecxStake();
        secxInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(secxInput);
    }
